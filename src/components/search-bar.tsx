...
export function SearchBar({
  ...
  const goToSearch = useCallback(
    (rawQ: string, rawOrt: string) => {
      ...
      if (ortRaw) {
        ...
        //             → kommun (kan väljas explicit via URL/autocomplete-pick i 2b)
  ...
  // === COMPACT — header-pill med bransch+ort horisontellt (mobile: stackad) ===
  if (variant === "compact") {
    return (
      <div ref={wrapRef} className="relative w-full max-w-xl">
        <form
          onSubmit={onSubmit}
          role="search"
          aria-label="Sök företag och ort"
        ...
        >
          ...
          <label className="group relative flex flex-1 items-center">
            <span className="sr-only">Företag, bransch eller person</span>
            <Search
              aria-hidden
            ...
            <input
              ...
              onKeyDown={onKeyDown}
              placeholder="Företag, bransch eller person"
              autoComplete="off"
              spellCheck={false}
              enterKeyHint="next"
