...
import { overlayGerSubstans, ordnaBoostade } from "@/lib/overlay-rankning";
...
export async function generateMetadata({
  ...
  return {
    ...
    //
    // En OVERLAY-KUND passerar grinden även när registerraden är naken. Utan
    // det här skulle en såld och publicerad profil kunna ligga kvar som
    // noindex: kunden har betalat för synlighet i Google och fått en sida som
    // uttryckligen ber Google att inte visa den. Uppslaget är cachat, så det
    // delar rundtur med sidan nedan.
    robots: robotsFor(
      foretagIsIndexable(f) || overlayGerSubstans(await getOverlayForForetag(f)),
    ),
  };
}

/**
 * Företagssidans hela innehåll.
 *
 * Bruten ur page.tsx för att förhandsvisningen ska kunna återanvända den.
 * Den publika sidan är ISR (revalidate 86400) för tiotusentals företag, och
 * hade den läst searchParams själv för att hitta `?preview=` hade varenda
 * sidvisning blivit en serverrendering. Nu betalar bara förhandsvisningen det
 * priset — se forhandsvisning/page.tsx.
 */
export default async function ForetagSida({
  slug,
  /** Satt ENDAST från förhandsvisningsrouten. Publik rendering skickar aldrig med den. */
  previewToken,
}: {
  slug: string;
  previewToken?: string;
  ...
  const cfarnr = parseCfarnrFromSlug(slug);
  if (!cfarnr) notFound();
  // getForetagByCfarnr filtrerar mot HANTVERK_BRANSCHER — företag utanför
  ...
  // Förhandsvisningen har en egen adress och en egen slug-form; en
  // omdirigering här hade kastat säljaren till den publika sidan.
  const canonicalSlug = foretagSlug(f);
  if (!previewToken && slug !== canonicalSlug) {
    permanentRedirect(`/foretag/${canonicalSlug}`);
  }
  ...
  // Overlay: den betalda profilen. I förhandsvisning läses UTKASTET i stället
  // för den publicerade raden — samma rendering, annan källa, så att säljaren
  // ser exakt vad kunden kommer att se.
  const [oboostadeRelated, sokord, overlay, overlays] = await Promise.all([
    kommun && f.ng1
      ? listRelatedForetag(kommun.code, f.ng1, f.cfarnr, 6)
      : Promise.resolve([] as Foretag[]),
    listSokordForCfarnr(f.cfarnr),
    previewToken
      ? getOverlayUtkastForForetag(f, previewToken)
      : getOverlayForForetag(f),
    loadAktivaOverlays(),
  ]);

  // "Liknande företag" är också en lista, och en betald placering gäller där
  // precis som i sök och i kommunlistan.
  const related = ordnaBoostade(oboostadeRelated, overlays);

  // Ett utkast som inte finns, eller en token som inte håller, ger ingen sida
  ...
  // featured. En kund som köpt framhävning ska se framhävd ut.
  const featured = featuredIRegistret || Boolean(overlay?.featured);

  /**
   * Vilket telefonnummer sidan visar i huvudet.
   *
   * `dolj_andra_nummer` är kundens köpta rätt att ersätta registrets nummer med
   * sitt eget — men bara NÄR DE FAKTISKT LÄMNAT ETT. Utan den spärren hade en
   ...
   *
   * Är rutan inte ikryssad visas registrets nummer i huvudet och kundens eget i
   ...
   *
   * Kundens egna ord (overlay `keywords` och `kategorier`) läggs FÖRE
   * registrets `sokordtable`-ord, och dubbletter faller bort skiftlägesokänsligt
   * — registret skriver "BERGSPRÄNGNING" där kunden skrivit "Bergsprängning",
   * och två varianter av samma ord i knowsAbout är brus, inte innehåll.
   */
  const strukturSokord = slaIhopSokord(
    [...(overlay?.kategorier ?? []), ...(overlay?.keywords ?? [])],
    sokord,
  );
  ...
  const localBusinessJsonLd = {
    ...
    ...(f.gatuadress || f.postort
      ...
      : {}),
    ...(visatTelefon ? { telephone: visatTelefon } : {}),
    ...(webb ? { url: webb } : {}),
    ...(f.epostadress ? { email: f.epostadress } : {}),
    ...(f.aeant ? { numberOfEmployees: f.aeant } : {}),
    ...(strukturSokord.length > 0
      ? {
          keywords: strukturSokord.join(", "),
          knowsAbout: strukturSokord.slice(0, 30),
        }
      : {}),
  };
    ...
    <div className="space-y-8">
      ...
      <header
        className={`rd-fade-up overflow-hidden rounded-3xl border bg-white rd-shadow-md ${
          featured ? "border-[var(--accent-cta)]/35" : "border-[var(--rule)]"
        }`}
      >
        <div className="relative p-6 sm:p-8">
          <div
            ...
            style={{
              background: featured
                ? "radial-gradient(ellipse 70% 50% at 0% 0%, rgba(242,116,12,0.16), transparent 65%),"
                  + "radial-gradient(ellipse 50% 40% at 100% 0%, rgba(10,77,140,0.08), transparent 70%),"
                  + "linear-gradient(180deg, #FFEAD1 0%, #ffffff 60%)"
                : "radial-gradient(ellipse 70% 50% at 0% 0%, rgba(10,77,140,0.10), transparent 65%),"
                  + "linear-gradient(180deg, #F2F6FB 0%, #ffffff 60%)",
            }}
          />
          <div className="relative flex flex-col gap-6 sm:flex-row sm:items-start">
            {logoUrl ? (
              ...
              <img
                ...
                alt={`${name} logotyp`}
                loading="lazy"
                className="size-20 shrink-0 rounded-2xl border border-[var(--rule)] bg-white object-contain p-2 shadow-[0_4px_12px_-4px_rgba(17,24,38,0.08)] sm:size-24"
              />
            ) : (
              <div
                aria-hidden
                className="inline-flex size-20 shrink-0 items-center justify-center rounded-3xl text-3xl font-bold text-white shadow-[0_8px_24px_-8px_rgba(242,116,12,0.55)] ring-1 ring-white/40 sm:size-24 sm:text-[2.25rem]"
            ...
            <div className="min-w-0 flex-1 space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                {featured && (
                  <span className="inline-flex items-center gap-1.5 rounded-full rd-brand-gradient px-2.5 py-1 text-[11px] font-semibold text-white shadow-[0_4px_10px_-4px_rgba(242,116,12,0.5)]">
              ...
              {size && (
                <div className="flex flex-wrap items-center gap-1.5 pt-1">
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-[var(--tint-3)] px-2.5 py-1 text-xs font-semibold text-[var(--accent-cta-ink)]">
      ...
      {/* ========= BETALD PROFIL (overlay) =========
          Ligger direkt under huvudet och ÖVER registerbeskrivningen: det kunden
          själv skrivit om sin verksamhet är mer värt för besökaren än en
          registerrad från 2011. Renderas bara när det finns en aktiv och giltig
          overlay-rad — eller, i förhandsvisning, ett utkast. */}
      {overlay && (
        <SponsradProfil overlay={overlay} namn={name} utkast={Boolean(previewToken)} />
      )}
        ...
        <div className="rd-fade-up rd-fade-up-delay-1 rd-card p-6 sm:p-7">
          ...
          {f.kontaktperson && (
            <div className="mt-5 flex items-center gap-3 border-t border-[var(--rule-soft)] pt-4">
              <span className="inline-flex size-9 items-center justify-center rounded-full bg-[var(--tint-2)] text-[var(--brand-ink)]">
            ...
            <>
              ...
              <div className="flex flex-wrap gap-2">
                {kommun && f.ng1 ? (
                  <Link
                    href={`/kommun/${kommun.slug}/${branschPageSlug(branschName, f.ng1)}`}
                    className="inline-flex items-center gap-1.5 rounded-full border border-[var(--brand)]/30 bg-[var(--tint-2)] px-3 py-1.5 text-sm font-medium text-[var(--brand-ink)] transition hover:bg-white"
                  >
                    {branschName}
                ...
                <Link
                  ...
                  className="inline-flex items-center gap-1.5 rounded-full border border-[var(--rule)] bg-white px-3 py-1.5 text-sm font-medium text-[var(--text-muted)] transition hover:border-[var(--accent-cta)]/40 hover:text-[var(--accent-cta-ink)]"
                >
                ...
                {sokord.map((s) => (
                  <li key={s}>
                    <Link
                      ...
                      className="inline-flex items-center rounded-full border border-[var(--rule)] bg-white px-2.5 py-1 text-[13px] font-medium text-[var(--text-body)] transition hover:border-[var(--accent-cta)]/45 hover:bg-[var(--surface-warm)] hover:text-[var(--accent-cta-ink)]"
                      title={`Sök ${s}`}
                    >
                      {s}
                    </Link>
        ...
        <div className="rd-fade-up rd-fade-up-delay-2 rd-card p-6 sm:p-7">
          ...
          <dl className="divide-y divide-[var(--rule-soft)]">
            <ContactRow
              ...
              label="Telefon"
              value={visatTelefon}
              href={visatTelefon ? `tel:${visatTelefon}` : null}
            />
            <ContactRow
              icon={<Mail className="size-4" aria-hidden />}
              label="E-post"
          ...
          {(visatTelefon || f.epostadress || webb || f.gatuadress) && (
          ...
          <div className="flex items-end justify-between gap-3">
            <div className="space-y-1">
              ...
              <h2 className="text-xl font-semibold text-[var(--text-strong)] sm:text-2xl">
                {branschName ? `${branschName} i ${kommun.name}` : `Hantverkare i ${kommun.name}`}
              </h2>
            </div>
            <Link
              href={
          ...
          <CompanyCardList>
            {related.map((rf) => (
              <li key={rf.id}>
                <CompanyCard
                  foretag={rf}
                  overlay={overlayFor(rf, overlays)}
  ...
  if (branschName) {
    items.push({
      ...
      a: `${name} är verksamt inom ${branschName.toLowerCase()}${
        kommun ? ` i ${kommun}` : ""
      }. För att se andra hantverksföretag i samma bransch kan du klicka på branschlänken ovan.`,
    });
  }
...
function ActionButton({
  ...
  const styles =
    variant === "primary"
      ? "bg-[var(--accent-cta)] text-white shadow-[0_6px_18px_-6px_rgba(242,116,12,0.55)] hover:bg-[var(--accent-cta-2)]"
      : variant === "secondary"
        ? "border border-[var(--rule)] bg-white text-[var(--text-strong)] hover:border-[var(--accent-cta)]/45 hover:bg-[var(--surface-warm)] hover:text-[var(--accent-cta-ink)]"
...
function ContactRow({
  ...
  return (
    <div className="flex items-center gap-4 py-3.5">
      <span className="inline-flex size-10 shrink-0 items-center justify-center rounded-xl bg-[var(--tint-2)] text-[var(--brand-ink)] ring-1 ring-[var(--brand)]/15">
        {icon}
      </span>
...
/**
 * Slår ihop två sökordslistor utan dubbletter, med den första listans ordning
 * och stavning bevarad. Jämförelsen är skiftlägesokänslig och trimmad.
 */
function slaIhopSokord(forst: string[], sedan: string[]): string[] {
  const ut: string[] = [];
  const sedda = new Set<string>();
  for (const ord of [...forst, ...sedan]) {
    const rent = ord.trim();
    if (!rent) continue;
